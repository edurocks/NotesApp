...
interface NoteDao {
    ...
    suspend fun getAllUnsycedNotes(): List<Note>

    @Query("SELECT * FROM locally_deleted_notes_ids")
    suspend fun getAllLocallyDeletedNoteIDs(): List<LocallyDeletedNoteID>

    @Query("DELETE FROM locally_deleted_notes_ids WHERE deletedNoteID = :deletedNoteID")
    suspend fun deleteLocallyDeletedNoteID(deletedNoteID: String)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertLocallyDeletedNoteIDs(locallyDeletedNoteID: LocallyDeletedNoteID)
